...
define([
  ...
  function(declare, lang, query, html, array, fx, all, Deferred, _WidgetsInTemplateMixin,
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      //if queryType is 1, it means that the query supports OrderBy and Pagination.
      //if queryType is 2, it means that the query supports objectIds, but
      //doesn't support OrderBy or Pagination.
      //if queryType is 3, it means that the query doesn't support objectIds.
      ...
      _onQueryListClicked:function(event){
        ...
        var filterUtils = new FilterUtils();
        this.currentAttrs.askForValues = filterUtils.isAskForValues(filterInfo);
        ...
        html.addClass(this.currentAttrs.queryTr, 'jimu-state-active');

        var callback = lang.hitch(this, function() {
          var layerDefinition = this.currentAttrs.queryTr.layerInfo;
          this.currentAttrs.layerInfo = layerDefinition;

          if(this._isServiceSupportsOrderBy(layerDefinition) &&
            this._isServiceSupportsPagination(layerDefinition)){
            this.currentAttrs.queryType = 1;
          }else if(this._isSupportObjectIds(layerDefinition)){
            this.currentAttrs.queryType = 2;
          }
          else{
            this.currentAttrs.queryType = 3;
          }
      ...
      _fromQueryListToQueryParams:function(){
        ...
        this.paramsDijit.build(layerUrl, this.currentAttrs.layerInfo, partsObj);
      ...
      _onBtnApplyClicked:function(){
        ...
        //set query.geometry
        this.currentAttrs.query.geometry = geometry;
        ...
        if(this.currentAttrs.queryType === 1){
          this._doQuery_SupportOrderByAndPagination(where, geometry);
      ...
      _onResultsScroll:function(){
        ...
        if(this.currentAttrs.queryType === 1){
          this._onResultsScroll_SupportOrderByAndPagination();
        }else if(this.currentAttrs.queryType === 2){
          this._onResultsScroll_SupportObjectIds();
        }
      },

      /*--------------------query support OrderBy and Pagination*/
      _doQuery_SupportOrderByAndPagination: function(where, geometry){
        ...
        defCount.then(lang.hitch(this, function(allCount){
          ...
          def.then(lang.hitch(this, function(response){
            ...
            all(promises).then(lang.hitch(this, function(results){
              ...
              this._addResultItems(features, resultLayer, results, relatedTableIds);
      ...
      _onResultsScroll_SupportOrderByAndPagination: function(){
      ...
      _getPopupFieldsWithFieldInfos: function(){
        var result = [];
        var allFieldInfos = lang.clone(this.currentAttrs.layerInfo.fields);
        var fieldInfosHash = {};
        array.forEach(allFieldInfos, lang.hitch(this, function(fieldInfo){
          fieldInfosHash[fieldInfo.name] = fieldInfo;
        }));
        var popupFields = lang.clone(this.currentAttrs.config.popup.fields);
      ...
      _addResultItems: function(features, resultLayer, relatedResults, relatedTableIds){
        ...
        array.forEach(features, lang.hitch(this, function(feature, i){
          ...
          //process attributes
          var attributes = feature.attributes;
          array.forEach(popupFields, lang.hitch(this, function(fieldInfo){
          ...
          var options = {
            feature: feature,
            titleTemplate: this.currentAttrs.config.popup.title,
        ...
        array.forEach(fieldInfosInAttrContent, lang.hitch(this, function(fieldInfo){
          ...
          if(specialType === 'image'){
            if(fieldValue && typeof fieldValue === 'string'){
              fieldValueInWidget = '<a href="' + fieldValue +
