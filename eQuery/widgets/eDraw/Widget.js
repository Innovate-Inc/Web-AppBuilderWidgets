	...
	return declare([BaseWidget, _WidgetsInTemplateMixin], {
		...
		_removeGraphic : function (graphic) {
			if (graphic.measure && graphic.measure.graphic) {
				graphic.measure.graphic.measureParent = false;
		...
		_getLengthAndAreaGeometryEngine : function (geometry, isPolygon, areaUnit, lengthUnit, wkid) {
			...
			// if(this.map_unitBaseFactor != 1){
			// result.length = result.length * this.map_unitBaseFactor;
			// if(result.area) result.area = result.area * this.map_unitBaseFactor * this.map_unitBaseFactor;
			// }
		...
		_setMeasureTextGraphic : function (graphic, result, existingMeasureGraphic) {
			...
			if (!this.showMeasure.checked) {
				if (graphic.measure && graphic.measure && graphic.measure.graphic) {
					//graphic.measure.graphic.measureParent = false; //Delete link between measure's label and this graphic
		...
		postCreate : function () {
			// var SR_info = geometryEngine.extendedSpatialReferenceInfo(this.map.spatialReference);
			// this.map_unitBaseFactor = (SR_info && SR_info.unitBaseFactor) ? SR_info.unitBaseFactor : 1;
